			...
			handler : function () {
				if ($("#incomeEditForm").form("validate")) {
					...
					if(!$("#incomeEditForm").form("validate")) {
						...
						$("#incomeEditForm").form({
						    url:"/Income/update/" + $("#income_incomeId_edit").val(),
						    onSubmit: function(){
								if($("#incomeEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	income_manage_tool = {
		...
		exportExcel: function() {
			$("#incomeQueryForm").form({
			    url:"/Income/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/Income/update/" + rows[0].incomeId,
					type : "get",
					data : {
