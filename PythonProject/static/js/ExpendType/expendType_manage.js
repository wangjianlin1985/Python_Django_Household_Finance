...
function initExpendTypeManageTool() {
	expendType_manage_tool = {
		...
		search: function() {
			$("#expendType_manage").datagrid("options").queryParams=queryParams; 
			$("#expendType_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#expendTypeQueryForm").form({
			    url:"/ExpendType/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/ExpendType/update/" + rows[0].expendTypeId,
					type : "get",
					data : {
