...
function initIncomeTypeManageTool() {
	incomeType_manage_tool = {
		...
		search: function() {
			$("#incomeType_manage").datagrid("options").queryParams=queryParams; 
			$("#incomeType_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#incomeTypeQueryForm").form({
			    url:"/IncomeType/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/IncomeType/update/" + rows[0].typeId,
					type : "get",
					data : {
