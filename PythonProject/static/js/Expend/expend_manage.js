			...
			handler : function () {
				if ($("#expendEditForm").form("validate")) {
					...
					if(!$("#expendEditForm").form("validate")) {
						...
						$("#expendEditForm").form({
						    url:"/Expend/update/" + $("#expend_expendId_edit").val(),
						    onSubmit: function(){
								if($("#expendEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	expend_manage_tool = {
		...
		exportExcel: function() {
			$("#expendQueryForm").form({
			    url:"/Expend/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/Expend/update/" + rows[0].expendId,
					type : "get",
					data : {
